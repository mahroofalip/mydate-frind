...
export default function ChatScreen({ route }) {
  ...
  const { conversation } = route.params; // ✅ removed non-serializable onMessagesRead param
  ...
  const markAsRead = useCallback(async () => {
    ...
    if (unreadMessages?.length > 0) {
      ...
      // ✅ Emit event for parent listeners (no non-serializable params)
      navigation.emit({
        type: 'messagesRead',
      ...
      .on(
        ...
        (payload) => {
          ...
          setMessages((prev) => {
            ...
            if (exists) return prev; // ✅ avoid duplicate key warning
