...
export default function WelcomeScreen({ navigation }) {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      const selfieExists = files?.some((file) =>
        file.name.startsWith("selfie_")
      );
      ...
      if (url.includes("access_token")) {
        ...
        if (params.access_token) {
          if (params.refresh_token) {
            ...
            // Just set access token manually
            await supabase.auth.setSession({
              access_token: params.access_token,
              refresh_token: "", // or omit it
            });
          }
        }
  ...
  return (
    <View style={styles.container}>
      <Image
        source={{
          uri: "https://w0.peakpx.com/wallpaper/882/58/HD-wallpaper-love-wedding-silhouette-sunset-thumbnail.jpg",
        }}
        style={styles.image}
        resizeMode="cover"
      />
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>
          Where hearts connect and love stories begin. Discover meaningful
